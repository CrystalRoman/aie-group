...
По итогам экспериментов лучшую метрику показала `TransformerUNet`, однако для воспроизводимой демонстрации API внутри git-репозитория используется `UNet`, так как её вес включён в проект, а более тяжёлые веса хранятся отдельно.  
...
### 3.3. На какой ОС проверялся проект

Основной сценарий установки, запуска тестов и поднятия API был проверен на **Windows** с использованием:
- `cmd` / PowerShell;
- `.bat`-скриптов из папки `scripts/`;
- локального открытия Swagger UI по адресу `http://127.0.0.1:8000/docs`.

Именно этот сценарий рекомендуется как основной для демонстрации проекта.
...
7. отдельно поясню, что:
   - по метрикам лучшей моделью была `TransformerUNet`;
   - в репозитории для демонстрационного API используется `UNet`;
   - веса `AttentionUNet` и `TransformerUNet` хранятся отдельно из-за размера и могут быть показаны отдельно.
