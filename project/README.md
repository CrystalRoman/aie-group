...
### 3.1. Почему в проекте нет готового `.venv`

Готовое виртуальное окружение **не хранится в репозитории**, потому что:
- оно тяжёлое;
- зависит от ОС и версии Python;
- плохо переносится между машинами;
- обычно не коммитится в git.

Вместо этого проект содержит:
- `requirements.txt`;
- `.env.example`;
- `.gitignore`;
...
### 3.2. Установка зависимостей локально
...
Если ты используешь подготовленные bat-скрипты:
...
Если раньше использовалась внешняя папка `data_full/`, в финальной версии проекта лучше:
- убрать `data_full/` из репозитория;
- перенести небольшой поднабор в `data/`;
- заменить активные пути в конфигах и демонстрационных ноутбуках на `data/...`.
...
3. `notebooks/service_api_check.ipynb`;
4. `GET /health`;
5. `POST /predict`;
