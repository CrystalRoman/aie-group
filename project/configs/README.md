...
### Примечание о сервисной конфигурации

В демонстрационной конфигурации API используется `configs/training_unet.yaml`, так как вес `UNet` включён в проект.
Веса `AttentionUNet` и `TransformerUNet` не включены в git-репозиторий из-за большого размера и при необходимости подключаются отдельно.
